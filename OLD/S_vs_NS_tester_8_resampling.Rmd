...
save_middle_panel <- function(covariate, residuals, outname, name_clean) {
  png(file = file.path("FIGURES/JUSTTHEMIDDLE3panelONE", sprintf("%s_middle_panel.png", outname)), width = 600, height = 600, res = 175)
...
try({
  ...
  x <- df$storm_year
  y <- df$mean_sea_level
  lo <- if (exists("lower")) get("lower") else NULL
  up <- if (exists("upper")) get("upper") else NULL
  ...
  # y-limits (respect lower/upper if present)
  ylim_vals <- c(y, lo, up)
  ylim_vals <- ylim_vals[!is.na(ylim_vals)]
  ylim_use <- if (length(ylim_vals)) range(ylim_vals) else range(y, na.rm = TRUE)

  # empty plot first
  plot(x, y, type = "n", xlab = "Year", ylab = "MSL [cm]", main = "",
       ylim = ylim_use)

  # points for all valid obs
  points(x[valid], y[valid], pch = 19, col = "blue")

  # contiguous 1-year runs → lines
  ...
  if (n >= 2) {
    ...
    while (i <= n) {
      j <- i
      while (j < n && (xi[j + 1] - xi[j]) == 1) j <- j + 1
      if ((j - i + 1) >= 2) {
        lines(xi[i:j], yi[i:j], col = "blue")
      }
      i <- j + 1
    }
  }
  ...
  try({
    plot(covariate, residuals, pch = 19, col = rgb(0, 0, 0, 0.5),
         #main = name_clean, xlab = "MSL [cm]", ylab = "Residual [cm]",
         xlab = "MSL [cm]", ylab = "Residual [cm]",
         ylim = range(c(residuals, lower2, upper2), na.rm = TRUE))
    polygon(c(newx2$covariate, rev(newx2$covariate)),
            c(lower2, rev(upper2)),
            col = rgb(1, 0, 0, 0.2), border = NA)
    lines(newx2$covariate, predict(fit_rlm2, newdata = newx2), col = "red", lwd = 2)
    legend("topright", legend = name_clean, cex = 1.2, bty = "o") 
  })
  ...
  try({
    ...
    #         main = "", xlab = "β [cm/cm]")
    #--------------------
    # Desired visible interval and bin count
    # Histogram settings
    x1 <- -3
    x2 <- 3
    n <- 81
    bin_width <- (x2 - x1) / n
    breaks <- seq(x1, x2, length.out = n + 1)
    
    # Clip data to desired histogram range
    data_clipped <- beta1_samples[beta1_samples >= x1 & beta1_samples <= x2]
    
    # Split by 0
    left_of_zero <- data_clipped[data_clipped < 0]
    right_of_zero <- data_clipped[data_clipped >= 0]
    
    # Calculate proportions
    pct_left <- length(left_of_zero) / length(data_clipped) * 100
    pct_right <- length(right_of_zero) / length(data_clipped) * 100
    
    # Decide message
    if (pct_right >= pct_left) {
      legend_text <- sprintf("Data < 0: %.1f%%", pct_left)
    } else {
      legend_text <- sprintf("Data >= 0: %.1f%%", pct_right)
    }
    
    # Plot
    hist(data_clipped, breaks = breaks,
         col = "darkorange", border = "black",
         main = "", xlab = "β [cm/cm]", xlim = c(x1, x2))
    abline(v=0,col='blue',lwd=3)
    # Add legend
    legend("topright", legend = legend_text, bty = "n", text.col = "black")
  })
  dev.off()
#  browser()
}
